...
const Avatar = React.forwardRef(function <C extends React.ElementType = 'div'>(
  ...
  return (
    ...
    >
      {src ? (
        ...
        <UserIcon className={cn(avatarIconVariants({ size }))} />
      )}
      {badgeContent && (
  ...
  props: PolymorphicAvatarProps<C> & { ref?: React.Ref<any> },
) => React.ReactElement;
...
const MultipleAvatarContainer: React.FC<IMultipleAvatarContainer> = ({
  children,
  lastElementSize = 'sm',
    ...
    <div className="flex items-center transition-transform duration-1000">
      ...
      {!showAllAvatars && remainingAvatars > 0 && (
        <div
          ...
          title="Show more"
          style={{
            transform: `translateX(-${maxVisibleAvatars * 10}px)`,
            zIndex: maxVisibleAvatars,
          }}
          onClick={handleShowAllAvatars}
        >
          <Avatar
            size={lastElementSize}
            title={`+ ${remainingAvatars}`}
            className={cn(showMoreAvatarVariants(), showMoreAvatarClassName)}
          />
        </div>
      )}
      {showAllAvatars && (
        <div
          ...
          title="Show less"
          style={{
            transform: `translateX(${React.Children.count(children) * 5}px)`,
        ...
        >
          <Avatar
            size={lastElementSize}
            className={cn(showLessAvatarVariants(), showLessAvatarClassName)}
            icon={<XIcon />}
          />
        </div>
      )}
