...
const InformationStatus: React.FC<IInformationStatus> = ({
  ...
  >
    <span className="mt-1">
      {(isHaveIcon || icon) &&
        (type === 'success'
          ? (icon ?? <SealCheckIcon className={cn(iconsStyle())} />)
          : type === 'warning'
            ? (icon ?? <SealWarningIcon className={cn(iconsStyle())} />)
            : type === 'error'
              ? (icon ?? <SealError className={cn(iconsStyle())} />)
              : type === 'info'
                ? (icon ?? <InfoIcon className={cn(iconsStyle())} />)
                : null)}
    </span>
    <span>{title}</span>
  </div>
