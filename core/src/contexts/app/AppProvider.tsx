...
const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  const [notificationTheme, setNotificationTheme] = useState<'colored' | 'lined' | 'default'>(
    'default',
  );
  const [notificationMode, setNotificationMode] = useState<'dark' | 'light'>('light');
  const [notificationAnimateMode, setNotificationAnimateMode] = useState<
    'bounce' | 'slide' | 'flip' | false
  >('bounce');
