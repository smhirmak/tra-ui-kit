...
export const loadVersionedComponent = (version: string, componentName: string) => {
  ...
  if (versionNum === 0) {
    return lazy(() =>
      import(`@/versions/v${version}/components/${componentName.toLowerCase()}.tsx`).catch(() => {
  ...
  return lazy(() => import(`@/components/${componentName.toLowerCase()}.tsx`));
};
...
export const loadVersionedPage = (version: string, pageName: string) => {
  return lazy(() =>
    import(`@/versions/v${version}/pages/${pageName}.tsx`).catch(() => {
