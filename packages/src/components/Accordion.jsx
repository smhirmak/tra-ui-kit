...
export const Accordion = ({ children, className, multipleExpand = false, variant = 'underlined' }) => {
  const [openIndexes, setOpenIndexes] = useState([]);
  const [openIndex, setOpenIndex] = useState(null);

  useEffect(() => {
    const initialOpenIndexes = [];
    let initialOpenIndex = null;

    React.Children.forEach(children, (child, index) => {
      if (React.isValidElement(child)) {
        const { defaultOpen } = child.props;
        if (defaultOpen) {
          if (multipleExpand) {
...
export const AccordionTrigger = ({ title, subTitle, onClick, isOpen, className, variant, disabled, startContent, icon, iconContainerClassName,
